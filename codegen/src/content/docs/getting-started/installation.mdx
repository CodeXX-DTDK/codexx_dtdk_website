---
title: Installation
description: Install the CodeXX DTDK Manager — the single entry point that installs, updates, and verifies codegen and every other DTDK component.
---
...
codegen ships as one component of the **CodeXX Dev-Tool Development Kit (DTDK)**. You don't download `codegen` directly. You install the **DTDK Manager** once — a small terminal app — and use it to install, update, and verify codegen and every other component. codegen is free and needs no license activation; the manager still handles that step for the DTDK components that require one.
...
<Tabs tabs={[
  ...
  <div>
    <LinkButton href="https://get.codexx-dtdk.com/api/download?platform=linux&arch=x64" variant="primary">Download DTDK Manager — Linux x64</LinkButton>
  ...
  <div>
    <LinkButton href="https://get.codexx-dtdk.com/api/download?platform=windows&arch=x64" variant="primary">Download DTDK Manager — Windows x64</LinkButton>
...
The manager is a terminal UI. Move with the arrow keys; press `Enter` on the **codegen** row to install it. `q` quits. The manager handles the download, checksum verification, and side-by-side version management for you.

## Licensing

codegen is free — there is no license tier, no activation step, and no `codegen license` command. Installing it through the manager is the whole story. (Other DTDK components still require activation inside the manager; that's unchanged.)
...
- The manager installs, updates, and verifies every component into `~/.codexx`, and handles license activation for the components that need one.
- codegen is free — no license, no activation step.
</Alert>
