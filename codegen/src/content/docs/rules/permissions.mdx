...
**Per-rule dotenv** at `<rulesDir>/<RuleName>/.env`. Standard format: `KEY=VALUE`, optional surrounding quotes, full-line `#` comments, blank lines ignored. A trailing `#` after a value on a `KEY=VALUE` line is **not** stripped — it stays part of the value. Loaded automatically when present; no allowlist required.
