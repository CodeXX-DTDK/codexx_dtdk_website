...
<Alert type="warning">
**One shared surface: dotenv.** Every rule's `.env`, the shared `.env`, and any `--env`
entries are merged into a **single process-wide map** before execution. A rule that
declares `permissions.env` therefore sees keys contributed by *every other rule* in the
same run, not just its own. Do not put a secret in a rule's `.env` on the assumption
that a co-running third-party rule cannot read it.
</Alert>
...
| Capability | Status |
|---|---|
| File system (`io.open`) | Blocked — no `io` library |
...
| Inter-rule communication | No shared Lua state — but see the dotenv caveat below |
