...
**Reading `node.annotations`.** The actual key is `attributes` (`node.attributes[i].ns`, `node.attributes[i].name`, `node.attributes[i].args`). Codex uses C++ attribute terminology throughout.
