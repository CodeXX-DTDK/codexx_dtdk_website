...
Before any rule runs, the engine builds a real semantic model over your source — not just a syntax tree. It indexes a symbol database, resolves a class hierarchy, and builds include and module dependency graphs. Rules can query this directly through the [native API](/reference/luau-globals/): resolve a base class to its declaring symbol, look a struct up by qualified name, or check for a member or method. The same engine computes deeper facts still — per-expression types, points-to sets, and, on debug-info builds, byte-level memory layout — that power the LSP and MCP surfaces. codegen deliberately hands each rule a clean syntactic AST node plus those symbol and hierarchy queries, rather than the full semantic model, so rules stay pure functions over JSON.
...
- The codegen CLI re-parses every input on each run. It does not use the workspace engine's persistent incremental index — that cache backs the long-running LSP and MCP servers, not the one-shot generator.
