...
- By default the codegen CLI re-parses every input on each run (the self-contained one-shot pipeline). Passing [`--use-workspace`](/integrations/cli/#preprocessor--parser) opts into the persistent ADR-033 cache that also backs the LSP and MCP servers: warm runs restore cached ASTs and skip re-parsing, and types resolve against a whole-workspace snapshot rather than a single invocation.
