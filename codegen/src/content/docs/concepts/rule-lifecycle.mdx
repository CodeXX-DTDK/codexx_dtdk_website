...
The engine resolves each `-a <name>` to `<rulesDir>/<RuleName>/` or `<rulesDir>/<vendor>/<RuleName>/` and reads: handler, preamble, optional grouping, optional `.env`, and `config.yaml` (with `extends:` chasing as needed). It also loads `<rulesDir>/../shared/*.luau` (alphabetical by filename, concatenated) as the flat prelude, reads `<rulesDir>/../shared/<vendor>/*.luau` as [components](/rules/anatomy/#components) — sources only; nothing runs until a script requires one — and `<rulesDir>/../shared/.env` as the baseline dotenv. CLI overrides (`-P`, `--env`, `--allow-http`, `--allow-env`) are folded in.

## Phase 2: Anchor scan + AST build

The engine scans every input file (under `-i`) for `// [[codegen::generated::<rule>::<qualifiedName>]]` markers (bare or `:begin]]`/`:end]]` form) and indexes them by `(rule, qualifiedName)`. It does not parse anything itself: the AST comes from the workspace daemon, which serves candidate nodes (`nodesWithAttribute`) and their subtrees (`getAstSubtrees`) over a local socket.
...
Files are written via a temp-file-and-rename pattern. Under `--dry-run`, contents are collected and emitted as a unified diff (zero context) to stdout — suitable for CI and LLM pipelines. With `--dry-run --show-tui`, the FTXUI viewer opens instead; accepting in the viewer writes the files. `--show-tui` without `--dry-run` is silently ignored.
