...
<Alert type="caution">
**🚧 Roadmap — WIP.** The `find_package(codegen)`, `codegen_rule()`, and `CODEGEN_OUTPUTS` helpers below are the design target for a planned `cmake_integration` package and are **not yet shipped**. The supported path today is `add_custom_command` driving the `codegen` binary; workspace analysis is unconditional (codegen always talks to the workspace daemon), so no flag or licence is needed to enable it. This page documents both the current and the planned path; track progress in the codegen changelog.
</Alert>
...
Workspace analysis is unconditional — codegen always connects to the workspace daemon serving `<root>/.codexx/daemon.sock`. Run codegen from the project root so the daemon's root matches, or pass `--workspace-dir` to point at a different directory.

## Planned: `find_package(codegen)` + `codegen_rule()`

A future package will install a CMake config module (`codegenConfig.cmake`) and a `codegen_rule()` helper that wraps the underlying `add_custom_command` and exposes the generated files for `target_sources`:
...
- The packaged helper is a roadmap item, not yet shipped; this page tracks the target.
</Alert>
