...
<Alert type="info">
`--debug` is free — no licence required. `LUAU_ENV_ENABLE_DEBUGGER` is a compile-time flag, but shipped builds always link the debug-enabled Luau environment, so the flag is effectively always on.
</Alert>
...
- Debugger support sits behind the `LUAU_ENV_ENABLE_DEBUGGER` compile-time flag, but shipped builds always link `luau_environment_dbg`, which defines it publicly — `--debug` is available in every shipped build.
