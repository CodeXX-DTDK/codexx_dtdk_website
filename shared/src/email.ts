...
// Per-tool branding. docsgen is unlicensed and sends no mail, but every
// licensed DTDK component (codegen today, more coming) reuses this emailer by
// binding its own brand once via createEmailer().
export interface EmailBrand {
  product: string // lowercase display + CLI name, e.g. "codegen"
  label: string // capitalized product label, e.g. "Codegen"
  from: string // RFC5322 From header, e.g. 'Codegen <noreply@codexx-dtdk.com>'
  siteUrl: string // brand/footer link → landing site
  appOrigin: string // where /activate lives (the tool's own subdomain)
}
...
export interface Emailer {
  ...
  sendPortalLinkEmail(params: { to: string; url: string; expiresAt?: string }): Promise<void>
}

export function createEmailer(brand: EmailBrand): Emailer {
  const { product, label, from: FROM, siteUrl: SITE, appOrigin: APP_ORIGIN } = brand
  ...
  async function sendActivationEmail(params: {
    ...
    const tierLabel =
      tier === 'team' ? 'Team' : tier === 'professional' ? 'Professional' : 'Community'
    ...
    const subject = trialDate
      ? `Your ${label} ${tierLabel} trial has started`
      : `Your ${label} ${tierLabel} license key`
...
<body style="margin:0;padding:0;background:#0f0f0f;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;color:#e0e0e0">
    ...
    <tr><td align="center" style="padding:24px 12px">
      <table role="presentation" class="container" width="560" cellpadding="0" cellspacing="0" style="width:100%;max-width:560px;background:#1a1a1a;border:1px solid #2a2a2a;border-radius:10px;overflow:hidden">
        ...
        <tr><td class="px py" style="background:#111;padding:22px 32px;border-bottom:1px solid #2a2a2a">
          <span style="font-size:18px;font-weight:700;letter-spacing:-0.02em;color:#fff">${product}</span>
        ...
        <tr><td class="px py" style="padding:32px">
          ...
          <p style="margin:0 0 16px;font-size:13px;line-height:1.6;color:#888">
            Open the <strong style="color:#cfd6cf">CodeXX DTDK manager</strong>, select the <strong style="color:#cfd6cf">${product}</strong> component and press Enter, then paste your license key into the activation pane. You can activate on ${machinesNote}.
          </p>

          <p style="margin:0;font-size:13px;line-height:1.6;color:#888">
            Already activated? Run <code style="color:#89b4fa;background:#0d0d0d;padding:2px 6px;border-radius:4px;font-size:12px">${product} license check</code> to confirm status from the CLI.
          </p>
        </td></tr>
  ...
  async function sendPortalLinkEmail(params: {
    to: string
    url: string
    expiresAt?: string
  }): Promise<void> {
    ...
    const { error } = await resend().emails.send({
      from: FROM,
      to,
      subject: `Your ${label} subscription portal link`,
      html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body style="margin:0;padding:0;background:#0f0f0f;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;color:#e0e0e0">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent">
    Manage your ${label} subscription — open the portal link below.
  </div>
  ...
  return { sendActivationEmail, sendPortalLinkEmail }
}
